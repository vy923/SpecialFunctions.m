function FCint = fresnelcx(X,fresnelType)
%
% ----------------------------------------------------
% MODS: Added +/-Inf checks, ONLY FOR TYPE 0
% note: Table gen may not work, use original supplied
% ----------------------------------------------------
%
...
klim = (X >= Xlim & ~isinf(X));                                             % MOD: omit inf vals
if any(klim(:))
...
FCint(isinf(X)) = 0.5;                                                      % MOD: limits at inf for 0-type
FCint(S) = - FCint(S);
